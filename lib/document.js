    ...
    switch (ext) {
      ...
      case 'docx': {
        ...
        return result.value;
         ...
         case 'pdf': {
        // Try pdfjs-dist first — handles more PDF variants including bad XRef
        try {
          ...
          if (pdfjsError.message === 'geen_tekst') {
            throw new Error(
              'Dit PDF-bestand bevat geen leesbare tekst — het is waarschijnlijk een gescande afbeelding. ' +
              'Exporteer het als DOCX of TXT.'
            );
          }
          throw new Error(
            'Kan tekst niet lezen uit dit PDF-bestand. ' +
            'Oplossing: open het PDF in Chrome → Afdrukken → "Opslaan als PDF" → upload die nieuwe versie. ' +
            'Of converteer het naar DOCX of TXT.'
          );
        }
      }
